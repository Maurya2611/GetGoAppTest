...
class EpisodeDetailViewController: UIViewController {
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        view.register(EpisodeDetailFeedCell.nib, forCellWithReuseIdentifier: EpisodeDetailFeedCell.reuseIdentifier)
...
extension EpisodeDetailViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if let count = viewModel?.selectedResult?.characters?.count, count > 0 && section == 1 {
            return count
        }
        return 1
    }
    func collectionView(_ collectionView: UICollectionView,
        ...
        if indexPath.section == 0 {
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: EpisodeDetailFeedCell.reuseIdentifier,
                                                                for: indexPath) as? EpisodeDetailFeedCell  else {
        ...
        if viewModel?.selectedResult?.characters?.count ?? 0 > 0 {
            if let item = viewModel?.selectedResult?.characters?[indexPath.row] {
                cell.bind(item)
            }
        } else {
...
extension EpisodeDetailViewController: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView,
                        layout collectionViewLayout: UICollectionViewLayout,
                        sizeForItemAt indexPath: IndexPath) -> CGSize {
        if indexPath.section == 0 {
            return CGSize(width: collectionView.frame.width - (padding * 2),
                          height: EpisodeDetailFeedCell.cellHeight)
        }
        return CGSize(width: collectionView.frame.width - (padding * 2),
